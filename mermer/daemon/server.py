...
from mermer.cmds.init_funcs import check_keys, mermer_init
from mermer.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from mermer.daemon.keychain_server import KeychainServer, keychain_commands
from mermer.daemon.windows_signal import kill
from mermer.plotters.plotters import get_available_plotters
from mermer.plotting.util import add_plot_directory
from mermer.server.server import ssl_context_for_root, ssl_context_for_server
from mermer.ssl.create_ssl import get_mozilla_ca_crt
from mermer.util.mermer_logging import initialize_logging
from mermer.util.config import load_config
from mermer.util.json_util import dict_to_json_str
from mermer.util.keychain import (
    Keychain,
...
from mermer.util.service_groups import validate_service
from mermer.util.setproctitle import setproctitle
from mermer.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Mermer.")
    quit()
...
service_plotter = "mermer_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "mermer": "mermer",
        "mermer_wallet": "start_wallet",
        "mermer_full_node": "start_full_node",
        "mermer_harvester": "start_harvester",
        "mermer_farmer": "start_farmer",
        "mermer_introducer": "start_introducer",
        "mermer_timelord": "start_timelord",
        "mermer_timelord_launcher": "timelord_launcher",
        "mermer_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    def _mermerpos_plotting_command_args(self, request: Any, ignoreCount: bool) -> List[str]:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        plotter: str = request.get("plotter", "mermerpos")
        command_args: List[str] = ["mermer", "plotters", plotter]

        command_args.extend(self._common_plotting_command_args(request, ignoreCount))

        if plotter == "mermerpos":
            command_args.extend(self._mermerpos_plotting_command_args(request, ignoreCount))
        elif plotter == "madmax":
    ...
    async def start_plotting(self, request: Dict[str, Any]):
        ...
        plotter = request.get("plotter", "mermerpos")
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("mermer daemon exiting in 5 seconds")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    if service_command == "mermer_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in mermer_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    mermer_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("mermer_daemon")
...
def main(argv) -> int:
    from mermer.util.default_root import DEFAULT_ROOT_PATH
