...
from mermer.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from mermer.plotters.mermerpos import get_mermerpos_install_info, plot_mermer
from mermer.plotters.madmax import get_madmax_install_info, plot_madmax
from mermer.plotters.install_plotter import install_plotter
...
def build_parser(subparsers, root_path, option_list, name, plotter_desc):
    ...
    for option in option_list:
        ...
        u_default = 0 if name == "mermerpos" else 256
...
def call_plotters(root_path: Path, args):
    ...
    build_parser(subparsers, root_path, mermer_plotter, "mermerpos", "Mermerpos Plotter")
    ...
    if args.plotter == "mermerpos":
        plot_mermer(args, mermer_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, mermer_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, mermer_root_path, root_path)
...
def get_available_plotters(root_path) -> Dict[str, Any]:
    ...
    mermerpos: Optional[Dict[str, Any]] = get_mermerpos_install_info()
    ...
    if mermerpos is not None:
        plotters["mermerpos"] = mermerpos
