...
from mermer.protocols import full_node_protocol, introducer_protocol
from mermer.protocols.protocol_message_types import ProtocolMessageTypes
from mermer.server.address_manager import AddressManager, ExtendedPeerInfo
from mermer.server.address_manager_store import AddressManagerStore
from mermer.server.outbound_message import NodeType, make_msg
from mermer.server.server import MermerServer
from mermer.types.peer_info import PeerInfo, TimestampedPeerInfo
...
class FullNodeDiscovery:
    ...
    def __init__(
        ...
        self.server: MermerServer = server
    ...
    async def on_connect(self, peer: ws.WSMermerConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSMermerConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSMermerConnection):
