...
from mermer.cmds.init_funcs import mermer_full_version_str
from mermer.protocols.protocol_message_types import ProtocolMessageTypes
from mermer.protocols.protocol_state_machine import message_response_ok
from mermer.protocols.protocol_timing import INTERNAL_PROTOCOL_ERROR_BAN_SECONDS
from mermer.protocols.shared_protocol import Capability, Handshake
from mermer.server.outbound_message import Message, NodeType, make_msg
from mermer.server.rate_limits import RateLimiter
...
from mermer.util.ints import uint8, uint16

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from mermer.util.network import class_for_type, is_localhost

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4


class WSMermerConnection:
    """
    ...
    async def perform_handshake(self, network_id: str, protocol_version: str, server_port: int, local_type: NodeType):
        if self.is_outbound:
            ...
            if inbound_handshake.network_id !=  'mermer'+network_id:
