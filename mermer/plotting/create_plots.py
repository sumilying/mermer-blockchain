...
from mermerpos import DiskPlotter

from mermer.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
from mermer.plotting.util import add_plot_directory
from mermer.plotting.util import stream_plot_info_ph, stream_plot_info_pk
...
from mermer.util.config import config_path_for_filename, load_config
from mermer.util.keychain import Keychain
from mermer.util.path import mkdir
from mermer.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk, master_sk_to_pool_sk
...
class PlotKeysResolver:
    ...
    async def get_farmer_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'mermer keys add', 'mermer keys generate' or provide a public key with -f"
            )
    ...
    async def get_pool_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'mermer keys add', 'mermer keys generate' or provide a public key with -p"
            )
