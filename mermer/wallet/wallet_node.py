...
from mermer.daemon.keychain_proxy import (
    KeychainProxy,
...
from mermer.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from mermer.protocols import wallet_protocol
from mermer.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from mermer.server.node_discovery import WalletPeers
...
from mermer.types.coin_spend import CoinSpend
...
from mermer.util.byte_types import hexstr_to_bytes
from mermer.util.check_fork_next_block import check_fork_next_block
from mermer.util.errors import Err, ValidationError
from mermer.util.ints import uint32, uint128
from mermer.util.keychain import Keychain
from mermer.util.lru_cache import LRUCache
from mermer.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from mermer.util.path import mkdir, path_from_root
from mermer.wallet.block_record import HeaderBlockRecord
from mermer.wallet.derivation_record import DerivationRecord
from mermer.wallet.settings.settings_objects import BackupInitialized
from mermer.wallet.transaction_record import TransactionRecord
from mermer.wallet.util.backup_utils import open_backup_file
from mermer.wallet.util.wallet_types import WalletType
from mermer.wallet.wallet_action import WalletAction
from mermer.wallet.wallet_blockchain import ReceiveBlockResult
from mermer.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'mermer keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSMermerConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSMermerConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSMermerConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSMermerConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSMermerConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSMermerConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSMermerConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
