...
from mermer.pools.pool_config import PoolWalletConfig, load_pool_config
from mermer.protocols import farmer_protocol, harvester_protocol
from mermer.protocols.pool_protocol import (
    ErrorResponse,
...
from mermer.server.server import ssl_context_for_root
from mermer.server.ws_connection import WSMermerConnection
from mermer.ssl.create_ssl import get_mozilla_ca_crt
from mermer.types.blockchain_format.proof_of_space import ProofOfSpace
from mermer.types.blockchain_format.sized_bytes import bytes32
from mermer.util.bech32m import decode_puzzle_hash
from mermer.util.byte_types import hexstr_to_bytes
from mermer.util.config import load_config, save_config, config_path_for_filename
from mermer.util.hash import std_hash
from mermer.util.ints import uint8, uint16, uint32, uint64
from mermer.util.keychain import Keychain
from mermer.wallet.derive_keys import (
...
from mermer.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self):
        ...
        if len(self.get_public_keys()) == 0:
            error_str = "No keys exist. Please run 'mermer keys generate' or open the UI."
            raise RuntimeError(error_str)

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["mmh_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["mmh_target_address"]
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]):
        ...
        if farmer_target_encoded is not None:
            ...
            config["farmer"]["mmh_target_address"] = farmer_target_encoded
        if pool_target_encoded is not None:
            self.pool_target_encoded = pool_target_encoded
            self.pool_target = decode_puzzle_hash(pool_target_encoded)
            config["pool"]["mmh_target_address"] = pool_target_encoded
    ...
    async def get_cached_harvesters(self, connection: WSMermerConnection) -> HarvesterCacheEntry:
