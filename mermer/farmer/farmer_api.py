...
from mermer.consensus.network_type import NetworkType
from mermer.consensus.pot_iterations import calculate_iterations_quality, calculate_sp_interval_iters
from mermer.farmer.farmer import Farmer
from mermer.protocols import farmer_protocol, harvester_protocol
from mermer.protocols.harvester_protocol import PoolDifficulty
...
from mermer.util.api_decorators import api_request, peer_required
...
class FarmerAPI:
    ...
    async def new_proof_of_space(
        self, new_proof_of_space: harvester_protocol.NewProofOfSpace, peer: ws.WSMermerConnection
    ):
        """
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            headers={"User-Agent": f"Mermer Blockchain v.{__version__}"},
                        ) as resp:
                            if resp.ok:
    ...
    @api_request
    @peer_required
    async def respond_plots(self, _: harvester_protocol.RespondPlots, peer: ws.WSMermerConnection):
