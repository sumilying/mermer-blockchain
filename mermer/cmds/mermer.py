...
from mermer.cmds.configure import configure_cmd
from mermer.cmds.farm import farm_cmd
from mermer.cmds.init import init_cmd
from mermer.cmds.keys import keys_cmd
from mermer.cmds.netspace import netspace_cmd
from mermer.cmds.passphrase import passphrase_cmd
from mermer.cmds.plots import plots_cmd
from mermer.cmds.show import show_cmd
from mermer.cmds.start import start_cmd
from mermer.cmds.stop import stop_cmd
from mermer.cmds.wallet import wallet_cmd
from mermer.cmds.plotnft import plotnft_cmd
from mermer.cmds.plotters import plotters_cmd
from mermer.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from mermer.util.keychain import set_keys_root_path, supports_keyring_passphrase
from mermer.util.ssl_check import check_ssl
...
@click.group(
    help=f"\n  Manage mermer blockchain infrastructure ({__version__})\n",
    epilog="Try 'mermer start node', 'mermer netspace -d 192', or 'mermer show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        from mermer.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
if not supports_keyring_passphrase():
    from mermer.cmds.passphrase_funcs import remove_passphrase_options_from_cmd

    # TODO: Remove once keyring passphrase management is rolled out to all platforms
    remove_passphrase_options_from_cmd(cli)


@cli.command("version", short_help="Show mermer version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs mermer daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by mermer start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from mermer.daemon.server import async_run_daemon
