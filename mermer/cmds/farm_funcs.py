...
from mermer.cmds.units import units
from mermer.consensus.block_record import BlockRecord
from mermer.rpc.farmer_rpc_client import FarmerRpcClient
from mermer.rpc.full_node_rpc_client import FullNodeRpcClient
from mermer.rpc.wallet_rpc_client import WalletRpcClient
...
from mermer.util.misc import format_bytes
from mermer.util.misc import format_minutes
from mermer.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total mermer farmed: {amounts['farmed_amount'] / units['mermer']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['mermer']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['mermer']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'mermer start wallet' and 'mermer wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'mermer wallet show'")
    else:
        print("Note: log into your key using 'mermer wallet show' to see rewards for each key")
