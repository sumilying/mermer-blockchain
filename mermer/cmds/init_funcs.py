...
from mermer.consensus.coinbase import create_puzzlehash_for_pk
from mermer.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_mermer_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from mermer.util.bech32m import encode_puzzle_hash
from mermer.util.config import (
    create_default_mermer_config,
    initial_config_file,
    load_config,
    save_config,
...
from mermer.util.path import mkdir
from mermer.util.ssl_check import (
...
from mermer.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
from mermer.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'mermer keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "mmh_target_address" not in config["farmer"]
    stop_searching_for_pool = "mmh_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("mmh_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("mmh_target_address"):
                stop_searching_for_pool = True

    # Set the destinations, if necessary
    updated_target: bool = False
    if "mmh_target_address" not in config["farmer"]:
        print(
            f"Setting the mmh destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["mmh_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["mmh_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['mmh_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "mmh_target_address" not in config["pool"]:
        print(f"Setting the mmh destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["mmh_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["mmh_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['mmh_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the MMH destination addresses, edit the `mmh_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    mermer_ca_crt, mermer_ca_key = get_mermer_ca_crt_key()
    mermer_ca_crt_path = ca_dir / "mermer_ca.crt"
    mermer_ca_key_path = ca_dir / "mermer_ca.key"
    write_ssl_cert_and_key(mermer_ca_crt_path, mermer_ca_crt, mermer_ca_key_path, mermer_ca_key)
    ...
    mermer_ca_crt, mermer_ca_key = get_mermer_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, mermer_ca_crt, mermer_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                mermer_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return mermer_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)


def mermer_version_number() -> Tuple[str, str, str, str]:
...
def mermer_minor_release_number():
    res = int(mermer_version_number()[2])
    print(f"Install release number: {res}")
    return res


def mermer_full_version_str() -> str:
    major, minor, patch, dev = mermer_version_number()
...
def mermer_init(
    ...
    if os.environ.get("CHIA_ROOT", None) is not None:
        print(
            ...
            f"Please unset the environment variable and run mermer init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Mermer directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if CHIA_ROOT is set, or if user has run mermer init twice
        # before a new update.
        if testnet:
    ...
    create_default_mermer_config(root_path)
    if testnet:
        configure(root_path, "", "", "", "", "", "", "", "", testnet="true", peer_connect_timeout="")
    create_all_ssl(root_path)
    if fix_ssl_permissions:
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'mermer keys show --show-mnemonic-seed'")
