...
import mermer.server.ws_connection as ws
from mermer.consensus.block_creation import create_unfinished_block
from mermer.consensus.block_record import BlockRecord
from mermer.consensus.pot_iterations import calculate_ip_iters, calculate_iterations_quality, calculate_sp_iters
from mermer.full_node.bundle_tools import best_solution_generator_from_template, simple_solution_generator
from mermer.full_node.full_node import FullNode
from mermer.full_node.mempool_check_conditions import get_puzzle_and_solution_for_coin
from mermer.full_node.signage_point import SignagePoint
from mermer.protocols import farmer_protocol, full_node_protocol, introducer_protocol, timelord_protocol, wallet_protocol
from mermer.protocols.full_node_protocol import RejectBlock, RejectBlocks
from mermer.protocols.protocol_message_types import ProtocolMessageTypes
from mermer.protocols.wallet_protocol import (
...
from mermer.server.outbound_message import Message, make_msg
from mermer.types.blockchain_format.coin import Coin, hash_coin_list
...
from mermer.types.mempool_item import MempoolItem
from mermer.types.peer_info import PeerInfo
...
from mermer.util.api_decorators import api_request, peer_required, bytes_required, execute_task, reply_type
from mermer.util.generator_tools import get_block_header
from mermer.util.hash import std_hash
from mermer.util.ints import uint8, uint32, uint64, uint128
from mermer.util.merkle_set import MerkleSet
...
class FullNodeAPI:
    ...
    async def request_peers(self, _request: full_node_protocol.RequestPeers, peer: ws.WSMermerConnection):
    ...
    async def respond_peers(
        self, request: full_node_protocol.RespondPeers, peer: ws.WSMermerConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: ws.WSMermerConnection
    ) -> Optional[Message]:
    ...
    @peer_required
    @api_request
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSMermerConnection) -> Optional[Message]:
        """
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: ws.WSMermerConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: ws.WSMermerConnection
    ) -> Optional[Message]:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: ws.WSMermerConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: ws.WSMermerConnection
    ) -> Optional[Message]:
        """
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: ws.WSMermerConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: ws.WSMermerConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: ws.WSMermerConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: ws.WSMermerConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (
    ...
    async def new_compact_vdf(
        self, request: full_node_protocol.NewCompactVDF, peer: ws.WSMermerConnection, request_bytes: bytes = b""
    ):
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def register_interest_in_puzzle_hash(
        self, request: wallet_protocol.RegisterForPhUpdates, peer: ws.WSMermerConnection
    ):
    ...
    async def register_interest_in_coin(
        self, request: wallet_protocol.RegisterForCoinUpdates, peer: ws.WSMermerConnection
    ):
