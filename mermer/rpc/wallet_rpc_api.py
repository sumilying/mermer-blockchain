...
from mermer.consensus.block_rewards import calculate_base_farmer_reward
from mermer.pools.pool_wallet import PoolWallet
from mermer.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
from mermer.protocols.protocol_message_types import ProtocolMessageTypes
from mermer.server.outbound_message import NodeType, make_msg
from mermer.simulator.simulator_protocol import FarmNewBlockProtocol
from mermer.types.blockchain_format.coin import Coin
from mermer.types.blockchain_format.sized_bytes import bytes32
from mermer.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from mermer.util.byte_types import hexstr_to_bytes
from mermer.util.ints import uint32, uint64
from mermer.util.keychain import KeyringIsLocked, bytes_to_mnemonic, generate_mnemonic
from mermer.util.path import path_from_root
from mermer.util.ws_message import WsRpcMessage, create_payload_dict
from mermer.wallet.cc_wallet.cc_wallet import CCWallet
from mermer.wallet.derive_keys import master_sk_to_singleton_owner_sk
from mermer.wallet.rl_wallet.rl_wallet import RLWallet
from mermer.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk, master_sk_to_wallet_sk
from mermer.wallet.did_wallet.did_wallet import DIDWallet
from mermer.wallet.trade_record import TradeRecord
from mermer.wallet.transaction_record import TransactionRecord
from mermer.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from mermer.wallet.util.trade_utils import trade_record_to_dict
from mermer.wallet.util.transaction_type import TransactionType
from mermer.wallet.util.wallet_types import WalletType
from mermer.wallet.wallet_info import WalletInfo
from mermer.wallet.wallet_node import WalletNode
from mermer.util.config import load_config
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "mermer_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "mermer_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("mmh_target_address")
        pool_target = config["pool"].get("mmh_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from mermer.pools.pool_wallet_info import initial_pool_state_from_dict
