...
- We have released version 1.0.0 of [chiapos](https://github.com/Mermer-Network/chiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `mmh_target_address` sections of config.yaml.
...
- calccrypto/uint128_t was updated in the Windows chiapos implementation. Mermerpos required some changes its build process to support MacOS ARM64.
...
- Changes were made to chiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run mermer-blockchain.
...
- Updated to chiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [chiapos release notes](https://github.com/Mermer-Network/chiapos/releases/tag/0.12.32).
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the mermer-blockchain, chiapos, chiavdf, chiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- There was a regression in Mermer Proof of Space ([chiapos](https://github.com/Mermer-Network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
...
- The libraries mermer-pos, mermer-fast-vdf, and mermer-bip-158 have been moved to their own repositories: [chiapos](https://github.com/Mermer-Network/chiapos), [chiavdf](https://github.com/Mermer-Network/chiavdf), and [chaibip158](https://github.com/Mermer-Network/chiabip158). They are brought in by mermer-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Mermer-Network/bls-signatures).
